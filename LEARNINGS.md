...
- 2026-05-24 #70: cmd/ entrypoints that import net/http are exempt from the domain-purity rule — reviewer should check net/http is in main.go/cmd/, not internal/<subsystem>/, rather than treating any net/http import as a blocking finding.
- 2026-05-24 #71: Track schema PRs that add only asset files + pure-domain packages require no curl evidence; the same LEARNINGS entry for pure-package PRs (#56) applies — cite the CI run URL in the test plan for tooling-verified ACs rather than leaving the evidence field empty.
- 2026-05-24 #72: bake-track reads only the first mesh/primitive's POSITION accessor AABB — this is documented as intentional for v1 but must be listed in non_goals[] in the plan so reviewers don't flag it as a gap when the glTF has multiple meshes.
- 2026-05-25 #73: suspGroundY equilibrium formula must be duplicated to cmd/physicswasm, cmd/replaygen, and test helpers — a single exported helper in physics would have prevented three separate SYNC NOTEs.
